...
**Ranking** is deterministic given the answer. Let `w = min(1, 2 · max(target confidence, lead over the next candidate))`. Then `score = w · (0.65 · P(target) + 0.20 · P(action matches kind)) + (1 − 0.85 · w) · fuzzy`, plus `0.25 · P(all) · P(match)` for rows in the set. Strong online judgments use the full semantic weighting; uncertain judgments retain more local relevance. Small probability-total differences from hundredth-place rounding are normalized; incomplete or invalid target distributions fall back locally. Without an answer the score is just `fuzzy`. Rows whose name you typed exactly always form a tier above the rest.

**Steadiness.** Local results update on every keystroke; Jev and Spotlight are asked 150 ms and 60 ms after typing pauses, and an identical question is never asked twice. While you extend or backspace a query, the previous answer stays in force at half weight, dimmed, until the new one arrives, so the list refines instead of snapping between local and online order. Only a fresh answer can light the green ↵ or offer a group row. Rows animate into their new positions, and the panel waits 120 ms before shrinking.
...
- **Index** (`LocalIndex.swift`): application bundles in standard and nested installation folders, Finder and the system Cryptex app folder; `~/Downloads`, `~/Desktop`, `~/Documents` themselves and their files (up to eight nested levels and 4,000 inspected entries per root, at most 250 newest entries per folder so one log directory cannot crowd out the rest, retaining 400 candidates by modified/added recency); user Shortcuts; nine system toggles; optional Chrome history (regular profiles, last 90 days, at most 3,000 URLs). Packages, hidden/private directories, symlink trees and cache/log/`node_modules` folders are excluded from file traversal. The last index is saved under `~/Library/Application Support/<bundle id>/index.json` and loaded at launch, so results appear before the first rebuild finishes. Folder and app scans are reused for 90 seconds; Chrome history is re-read only when its database files change. With Spotlight enabled, files you opened in the last 30 days anywhere in your home folder join the index too.
...
- **Fuzzy prefilter** (`Fuzzy.swift`): exact, prefix, word-initial, single-edit typo and subsequence scoring over titles, keywords and the two enclosing folder names, tokenized once per item so a keystroke over thousands of candidates costs about a millisecond. Starting the first word of a name scores higher than starting a later word. Natural-language filler is stripped, including a filler word still being typed. Your own history adds to the score: an item you opened for this exact query before wins outright (a learned alias, even when the words differ), typing the start of a short learned query lifts its item, and pinned, recently or often opened items win ties. Among browser links with the same score, the hub comes first: shallower paths before deep links with record ids and query strings, then more often visited, then more recent. So `stripe dashboard` offers the dashboard home rather than the coupon you looked at last, while `stripe coupons` still goes straight to that coupon. Time-window queries keep newest-first order.
...
- **Chrome only, and only visits.** Safari's history is not read (it needs Full Disk Access); the Chrome `downloads` table and open tabs are not used. History is re-read when the panel opens and Chrome's database has changed, so a page visited seconds ago appears on the next ⌥Space. Folder scans are reused for 90 seconds; a file saved seconds ago is still found through the live Spotlight search.
